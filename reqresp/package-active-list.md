curl 'https://putri-veronica.my.id/api/user/limited/xl/package-active-list.json' \
  -H 'accept: */*' \
  -H 'accept-language: en-US,en;q=0.6' \
  -H 'content-type: application/json' \
  -H 'origin: https://putri-veronica.my.id' \
  -H 'priority: u=1, i' \
  -H 'referer: https://putri-veronica.my.id/limited/tembak-paket-xl' \
  -H 'sec-ch-ua: "Brave";v="137", "Chromium";v="137", "Not/A)Brand";v="24"' \
  -H 'sec-ch-ua-mobile: ?0' \
  -H 'sec-ch-ua-platform: "Linux"' \
  -H 'sec-fetch-dest: empty' \
  -H 'sec-fetch-mode: cors' \
  -H 'sec-fetch-site: same-origin' \
  -H 'sec-gpc: 1' \
  -H 'user-agent: Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/137.0.0.0 Safari/537.36' \
  -H 'x-token: eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..FFtPvvJos5Wz-rcNNdIUj2plb_7RHW8baSVl299fw25A2ROZOC-q00MgcEJJBnJCq_KcskUF6AEnPMkVFv8cyieYWEeFELCHmBCLJi2jqkzQVkrPgR7DNmjbpzFnwKK_iMWFiej15o9TbK8gDNLL3b4S7zuU5PZBnraqiL1tm9Zx--K2kotNSOQ225ggI_7ca3BwpDjeKu1nF-iZMbMlgwd69gOkmonnIHHQeU8C2C2CQxfj3KazAv95Ho4_BT2yniptczzYI_xEg_Rk_yt51N1hdFd1LcLaZNJ1zhRCN9wnr6t_tKZRbstZdfWpfk03tdRaCKsNAZlpBMHC_CMBSA' \
  --data-raw '{"access_token":"1097498:7c17952a-343a-49b1-9e2e-4a52b7682b7a"}'



{
    "statusCode": 200,
    "message": "SUCCESS",
    "success": true,
    "data": {
        "msisdn": "6287817739901",
        "text": "Berikut ini adalah daftar paket/kuota kamu yang aktif saat ini untuk nomor XL 6287817739901. Penting: Mungkin paket tidak langsung muncul setelah beberapa detik pembelian paket, tunggu hingga 1 menitan dan klik ulang tombol Cek Paket untuk melihat update hasilnya.",
        "quotas": [
            {
                "encrypted_package_code": "Oar0TxR5C2IDUm6QN9vBEJHEuYh6tAPmS7eAkxzVB5a1QgPsy4e-3G_Ll2T3OESm_NkQNrv3ow2O0EJYbvJIKPxJNAcc2K-yX-Jwf19zXG_sBhI2FNHY-UfS9FUfp-az",
                "name": "Xtra Unlimited Turbo Vidio",
                "expired_at": "2025-09-17 23:59",
                "benefits": []
            }
        ]
    }
}
